...
SSDP.prototype._init = function (opts) {
  ...
  this._ipPort = this._ssdpIp + (this._ssdpPort==''?'':':' + this._ssdpPort)

  this._ttl = opts.ttl || 1800

  this._description = opts.description || (opts.description==''?'':'upnp/desc.html')

  this._usns = {}
  this._udn = opts.udn || 'uuid:bb395318-fd56-430e-883f-430dea6b42f4'
}
...
SSDP.prototype.server = function (ip, portno) {
  ...
  if (portno == undefined) portno = '10293'
  ...
  if (!ip || ip == '0.0.0.0') {
    dns.lookup(os.hostname(), function (err, add) {
      self._httphost = 'http://' + add + (portno==''?'':':' + portno)
      bind()
    })
  } else {
    this._httphost = 'http://' + ip + (portno==''?'':':' + portno)
    bind()
  }
